...
def create_general_btns(player: Player) -> tk.Frame:
    global frame
    # general button frame to hold the ff btn: [status, level up]
...
def create_skills_btns(current_player: Player, enemy_player: Player) -> tk.Frame:
    ...
    # warrior
    warrior_btn = tk.Button(active_skill_btns, text="slash",
                            command=lambda: current_player.use_skill("slash", enemy_player))
    warrior_btn.grid(row=0, column=0, sticky=tk.W + tk.E)
    # assassin
    assassin_btn = tk.Button(active_skill_btns, text="stab",
                             command=lambda: current_player.use_skill("stab", enemy_player))
    assassin_btn.grid(row=0, column=1, sticky=tk.W + tk.E)
    # mage
    mage_btn = tk.Button(active_skill_btns, text="fireball",
                         command=lambda: current_player.use_skill("fireball", enemy_player))
    mage_btn.grid(row=0, column=2, sticky=tk.W + tk.E)
    # no class
    no_class_btn = tk.Button(active_skill_btns, text="punch",
                             command=lambda: current_player.use_skill("difficult punch", enemy_player))
